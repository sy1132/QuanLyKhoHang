...
namespace QLKhoHang.Controllers
{
    public class SupplierController : Controller
    {
        ...
        {
            _context = context;
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial; // Cấu hình EPPlus
        }
        ...
        public async Task<IActionResult> Create([Bind("Name,Address,Phone,Email,WarehouseId")] Supplier supplier)
        {
                ...
                using (var package = new ExcelPackage(stream))
                {
                    ...
                    for (int row = 2; row <= rowCount; row++)
                    {
                        var supplier = new Supplier
                        {
                            ...
                            WarehouseId = int.TryParse(worksheet.Cells[row, 5].Value?.ToString(), out int warehouseId) ? warehouseId : 0,
        ...
        public async Task<IActionResult> Export()
        {
            ...
            using (var package = new ExcelPackage())
            {
                ...
                {
                    worksheet.Cells[i + 2, 1].Value = suppliers[i].Name;
                    worksheet.Cells[i + 2, 2].Value = suppliers[i].Address;
                    worksheet.Cells[i + 2, 3].Value = suppliers[i].Phone;
                    worksheet.Cells[i + 2, 4].Value = suppliers[i].Email;
