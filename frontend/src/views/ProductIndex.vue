    ...
    <div>
      <div class="header-bar">
        ...
        <div class="table-actions">
          ...
          <router-link to="/ProductAdd" class="btn green"><span class="plus-btn">+</span> Thêm mới</router-link>
          <button class="btn green" @click="triggerFileInput">
            <span class="icon">&#128190;</span> Import
          </button>
          <button class="btn green" @click="exportExcel"><span class="icon">&#128190;</span> Xuất file</button>
    ...
    <div class="content-layout">
      ...
      <div class="sidebar">
        <div class="filter-box">
          ...
          <div class="filter-option"><input type="checkbox" /> Hàng hóa</div>
          <div class="filter-option"><input type="checkbox" /> Dịch vụ</div>
          <div class="filter-option"><input type="checkbox" /> Combo - Đóng gói</div>
        </div>
        <div class="filter-box">
          <div class="filter-title">
            Nhóm hàng <span class="plus">+</span>
          </div>
        ...
        <div class="filter-box">
          ...
          <div class="filter-option"><input type="radio" name="tonkho" checked /> Tất cả</div>
          <div class="filter-option"><input type="radio" name="tonkho" /> Dưới định mức tồn</div>
        </div>
      </div>
      <!-- Main content -->
      <div class="main-content">
        ...
        <div class="table-container">
          <table>
            ...
            <tbody>
              <tr v-for="product in filteredProducts" :key="product.id" @click="$router.push(`/ProductDetail/${product.id}`)">
                <td><input type="checkbox" /></td>
                <td v-if="isColVisible('image')">
                  <img :src="product.image || defaultImg" alt="img"
                    style="width:40px;height:40px;object-fit:cover;border-radius:4px;" />
                </td>
                <td v-if="isColVisible('barcode')">{{ product.barcode }}</td>
                <td v-if="isColVisible('name')">{{ product.name }}</td>
                <td v-if="isColVisible('category')">{{ product.categoryID }}</td>
                ...
                <td v-if="isColVisible('modifiedDate')">{{ formatDate(product.modifiedDate) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  methods: {
    async fetchProducts() {
      try {
        ...
        // Nếu không có trường price, bạn có thể tính hoặc bỏ
        this.products = res.data.map(p => ({ ...p, price: p.price || p.cost * 1.2 }));
    ...
    async exportExcel() {
      try {
        const response = await axios.get("https://localhost:7189/api/Products/export-excel", {
          responseType: "blob",
        });
